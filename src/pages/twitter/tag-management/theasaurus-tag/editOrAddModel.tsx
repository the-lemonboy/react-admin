...
import twitterService, { AddAreaReq } from '@/api/services/twitterService';
...
function EditorOrAddModel({
  ...
  const createPlanetMutation = useMutation({
    mutationFn: async (params: AddAreaReq) => {
      const res = await twitterService.AddArea(params);
      return res.data;
    },
    onSuccess: () => {
  ...
  const updateAreaMutation = useMutation({
    mutationFn: async (params: AddAreaReq) => {
      const res = await twitterService.UpdateArea(params);
      return res.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries(['planetAreaList']);

...
import twitterService, { ChangeAreaStatusReq } from '@/api/services/twitterService';
...
export default function ThesaurusTag() {
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="flex w-full justify-center text-gray">
          <Button type="primary" onClick={() => onEditAreaTag(record)}>
            编辑
          </Button>
        </div>
      ),
  ...
  const changeTheasaurusStatus = useMutation({
    mutationFn: (params: ChangeAreaStatusReq) => {
      const res = twitterService.ChangeAreaStatus(params);
      return res;
    },
    onSuccess: () => {
